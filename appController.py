...
class AppController(threading.Thread):
    def __init__(
        ...
        event,
        observerModel,
        myResultFrameQueue_Model_Controller,
        myResultFrameQueue_Controller_Model,
    ):
        ...
        self.event = event
        self.observerModel = (
            observerModel  # AppController acts as Observable, observer is appModel
        )
        self.observerView = None  # AppController acts as 2nd Observable, observer appView is registered after it is availalbe
        self.brightness = 0
        self.cornersTableData = (
            None  # calibration matrix of the four points on the table
        )
        ...
        mixer.init()
        self.voiceCmdSpeaker = (
            "espeak -v english-us -a 190 -p 70 -s 120 -m '{}' --stdout | aplay & "
        )
        ...
        self.dataTOH = (
            []
        )  # informations of the user inputs for the tower of hanoi from appView
        self.stateTOH = (
            []
        )  # Contains the results of the tower of hanoi algorithm about which disc was moved from which position to which position.
        self.counter_TOH = 0  # the number of moves during tower of hanoi game

        ##### checkers game
        self.cornersdataCheckers = (
            None  # calibration matrix to zoom on checkers game board
        )
        ...
        self.modelCheckers = self.load_modelCheckers(
            MODEL_CHECKERS
        )  # load trained yolov5 model for detection with bounding-box of checkers pieces
        self.classesCheckers = (
            self.modelCheckers.names
        )  # load labels of checkers pieces (white or black)
        self.dictCheckers = (
            {}
        )  # to store the pixel coordinates of the centers from the 64 fields of the game board
        ...
        self.viaPiece = (
            []
        )  # saved the pixel coordinates of the player's pieces that have been jumped
        ...
        self.choose_user = (
            True  # set of true if the user play and false if the camputer play
        )
        ...
        self.last_time = 0
        self.write_comp = (
            False  # help to write computer symbol on virtual tic-tac-toe gameboard
        )
        self.board = [" " for x in range(9)]  # create the tic-tac-toe gameboard
        self.stopgameticTT = False  # to stop the checkers game
        self.center_coordPixel = (
            None  # to save the pixel coordinate of center of a tic-tac-toe field
        )
        self.cornersdataTicTT = (
            None  # calibration matrix to zoom on tic-tac-toe gameboard
        )
        self.modelTicTT = load_model(
            MODEL_TIC_TAC_TOE
        )  # load trained keras model for classification of tic-tac-toe pieces
        ...
        self.counterIndexTicTT = (
            []
        )  # to copy the current arrays of the tic-tac-toe game field and verify if the the new array differt from this copy. If this case then there is a new piece from the computer in the game board.
        self.stopNotifyObserverModel_TicTT_HomeView = (
            False  # set true to stop notifying the model
        )
    ...
    def updateHomeView(
        self, brightness=0, stopFPSHomeView=False, cornersTableData=None, appName=None
    ):
        """
    ...
    def notifyObserverModel_TOH(self):
        ...
        self.observerModel.update_TOH(
            counter_TOH=self.counter_TOH,
            dataTOH=self.dataTOH,
            stateTOH=self.stateTOH,
    ...
    def startGameTOH(self):
        ...
        """
        logger.debug(
            "AppController Tower of Hanoi: tower of hanoi game has been started!"
        )
        self.hanoi(
            self.dataTOH[0], self.dataTOH[1], self.dataTOH[2], self.dataTOH[3]
        )  # get Solving information of the tower of hanoi for the Model
        ...
        try:
            _thread.start_new_thread(
                ...
                (),
            )  # start in a new Thread a the new view of Problem-Solving Progression
        except:
            logger.error("Exit Tower of Hanoi from appController !")
        self.dataTOH = (
            []
        )  # if the robot finisched to solve the current tower of hanoi problem, set all the variable to empty
        self.stateTOH = []
        self.counter_TOH = 0
        # self.appName = None
    ...
    def load_modelCheckers(self, model_name):
        ...
        """
        logger.debug(
            "AppController checkers game: load model for the detection of checkers pieces"
        )
        model = torch.hub.load(
            DIRECTORY_MODEL_CHECKERS, "custom", path=model_name, source="local"
        )
        return model
    ...
    def notifyObserverModel_Checkers(self):
        ...
        """
        logger.debug(
            "AppController checkers game: send configurations values (transformation matrix and function to hide FPS to model"
        )
        self.observerModel.update_Checkers(
            cornersdataCheckers=self.cornersdataCheckers,
            stopFPSCheckers=self.stopFPSCheckers,
            appName=self.appName,
        )
    ...
    def notifyObserverModel_Checkers_Robot(self):
        ...
        """
        logger.debug(
            "AppController checkers game: send the pixel coordinates to Model about the movents of computer in game board"
        )
        self.observerModel.update_Checkers_Robot(
            xy_coord_From=self.xy_coord_From,
            xy_coord_To=self.xy_coord_To,
            viaPiece=self.viaPiece,
        )
    ...
    def startGameCheckers(self):
        ...
        self.mat = [
            ...
            [],
            [],
        ]  # initializes an empty array to save the initial positions of pieces for the checkers game
        ...
        self.dictionaryCheckers = (
            {}
        )  # is used to compare if the position of player pieces from the last saved arrays and the position of player pieces from the current detected arrays are same or not.
        self.dictionaryCheckersPiecesFrom = (
            {}
        )  # to get the start position (x,y) of the player piece who has been moved to send it to checkers algorithm
        self.dictionaryCheckersPiecesTo = (
            {}
        )  # # to get the end position (x,y) of the player piece who has been moved to send it to checkers algorithm
        ...
        if self.matrix is None:
            logger.debug(
                "AppController checkers game: a new checkers Game is starting ..."
            )
            self.matrix = np.array(
                self.mat
            )  # convert arrays of game field in numpy arrays
            self.checkers = checkers.Checkers(
                self, self.matrix
            )  # AppController acts as Observable, observer is Checkers
        else:
            logger.debug(
                "AppController checkers game: a checkers part is continuing ..."
            )
        ...
        self.scoreCheckers = [
            0,
            0,
        ]  # initialize score of payer with index 0 and Franka robot with index 1
        _thread.start_new_thread(
            self.doAudio, ("Welcome to Checkers Game",)
        )  # play welcome message with audio during the starting of checkers game
        _thread.start_new_thread(
            self.doAudio, ("you can play now", True)
        )  # the second argument (True) defines a timeout of 3 seconds so that the new text message can been played to notify the player that he can play now
        self.notifyObserverViewCheckers(
            newScore=self.scoreCheckers
        )  # send the initial score to view
    ...
    def continueGameCheckers(self):
        ...
        """
        logger.debug(
            "AppController checkers game: continue a saved checkers game state"
        )
        if os.path.isfile(
            SAVE_STATE_CHECKERS_GAME
        ):  # verify if the file "data/continue_save_state_Checkers.npy" exists
            arr = np.load(
                SAVE_STATE_CHECKERS_GAME, allow_pickle=True
            )  # if the file exists load this
            if len(arr) != 0:
                self.matrix = arr[
                    1
                ]  # load the saved matrix of a saved checkers game state and copy this into self.matrix
                self.startGameCheckers()  # start a new game with the saved state of checkers game
            else:
                logger.error(
                    "AppController checkers game: the file `continue_pause.npy` is corrupted"
                )
                self.notifyObserverViewCheckers(
                    message="Please delete manually the file `continue_pause.npy` in Folder `data` and try again!"
                )
        else:
            logger.error(
                "AppController checkers game: the file `continue_pause.npy` don't exist"
            )
            self.notifyObserverViewCheckers(
                message="Can't find any saved checkers game state. Please start a new game and save it state!"
            )
    ...
    def saveStateGameCheckers(self):
        ...
        if self.matrix is None:
            logger.error(
                "AppController checkers game: the game state can't be saved if the matrix is None"
            )
            self.notifyObserverViewCheckers(
                message="Please start a new Game and save it state !"
            )
        else:
            ...
            self.notifyObserverViewCheckers(
                message="Current checkers game state has been saved ..."
            )
    ...
    def stopCheckers(self):
        ...
        """
        logger.debug(
            "AppController checkers game: current checkers game has been stopped!"
        )
    ...
    def notifyObserverViewCheckers(self, newScore=None, message=None):
        ...
        """
        logger.debug(
            "AppController checkers game: notify view with current scores and notifications of the game process"
        )
        self.observerView.updateCheckersView(newScoreCheckers=newScore, message=message)

    def updateCheckers(
        self,
        stopFPSCheckers=False,
        levelCheckers=1,
        cornersdataCheckers=None,
        appName=None,
        getCenterofFieldsCheckers=None,
        use_getCenterofFields=1,
    ):
        """
    ...
    def getPlayerCheckers(self, points, frame):
        ...
        :return: None
        """
        self.indexs1 = np.flatnonzero(
            np.char.startswith(self.matrix, "b")
        )  # get index of the value, that beginnt with b (player piece) from the current gameboard
        results = self.score_frame(frame)  # get label with pixel coordinates

        self.arr = (
            []
        )  # define array to save the current state of gameboard with the camera

        self.curr_state = np.array(
            [["---" for col in range(8)] for row in range(8)]
        )  # create a leer (---) array game board
        ...
        for k in range(n):
            ...
            if row[4] >= 0.5:
                x1, y1, x2, y2 = (
                    ...
                    int(row[2] * x_shape),
                    int(row[3] * y_shape),
                )  # get the pixels coordinates of the detected classe
                x1, y1, x2, y2 = (
                    x1 + 5,
                    y1 + 5,
                    x2 - 5,
                    y2 - 5,
                )  # reduce border of the bounding box
                self.array1.append(
                    [(x1, y1, x2, y2), self.class_to_label(labels[k])]
                )  # save the pixels coordinates of the detected game piece on frames and the string of this label
        ...
        for i in range(len(points)):
            ...
            for rect in self.array1:
                result = self.rectContrains(
                    (rect[0][0], rect[0][1]), (rect[0][2], rect[0][3]), points[i]
                )
                if (
                    result == True
                ):  # if center point inside the bounding box, then get the string of label of this bounding box and write this into the array self.arr
                    label = rect[1]
                    self.arr.append(label)
                    break
            if (
                result == False
            ):  # if none bounding box, then the field in the gameboard has not a piece (add None when not piece detected into a field)
            ...
            for i in range(8):
                for j in range(8):
                    if self.arr[count] == "white":
                        ...
                        pass
                    coord = points[count]
                    self.dictCheckers["{i}{j}".format(i=i, j=j)] = (
                        coord  # save the center coordinate of the i,j-position of numpy array curr_state
                    )
                    count += 1
        except:
            logger.error(
                "AppView: error by created the array of the current gameboard state in the getPlayerCheckers()-function"
            )

        self.indexs2 = np.flatnonzero(
            np.char.startswith(self.curr_state, "b")
        )  # get index of the value, that beginnt with b (player piece) from the detected gameboard by the camera

        compare = np.array_equal(
            self.indexs1, self.indexs2
        )  # compare these 2 arrays (if True then equal, if False then not equal)
        ...
        if (
            self.dictionaryCheckers[maxCounted] == 2
        ):  # if the same value comes two time, then verify if the value is False or True
            if (
                maxCounted == False
            ):  # if False, then the two state array gameboard are different
                try:
                    diff_value_from = np.setdiff1d(
                        self.indexs1, self.indexs2
                    )  # get the  difference of two arrays and return the unique values in self.indexs1 that are not in self.indexs2. the different value is the position of the player piece that has been moved.
                    diff_value_to = np.setdiff1d(
                        self.indexs2, self.indexs1
                    )  # cible position of the moved piece

                    # extract and prepare informations for the algorithm
                    fromInd1 = str(self.matrix.flatten()[diff_value_from][0])[
                        1:2
                    ]  # extract the x pixel coordinate of the start position of the player piece that has been moved.
                    fromInd2 = str(self.matrix.flatten()[diff_value_from][0])[
                        2:3
                    ]  # extract the y pixel coordinate of the start position of the player piece that has been moved.
                    toInd1 = str(self.curr_state.flatten()[diff_value_to][0])[
                        1:2
                    ]  # extract the x pixel coordinate of the cible position of the moved player piece.
                    toInd2 = str(self.curr_state.flatten()[diff_value_to][0])[
                        2:3
                    ]  # extract the y pixel coordinate of the cible position of the moved player piece.

                    fromInd = fromInd1 + "," + fromInd2
                    toInd = toInd1 + "," + toInd2

                    # use a dictionary to count the value of start position and cible position of the moved piece
                    self.dictionaryCheckersPiecesFrom[fromInd] = (
                        self.dictionaryCheckersPiecesFrom.get(fromInd, 0) + 1
                    )
                    self.dictionaryCheckersPiecesTo[toInd] = (
                        self.dictionaryCheckersPiecesTo.get(toInd, 0) + 1
                    )
                    maxCountedPiecesFrom = max(
                        self.dictionaryCheckersPiecesFrom,
                        key=self.dictionaryCheckersPiecesFrom.get,
                    )
                    maxCountedPiecesTo = max(
                        self.dictionaryCheckersPiecesTo,
                        key=self.dictionaryCheckersPiecesTo.get,
                    )

                    # if the same value of the start position and cible position comes 2 times
                    if (
                        self.dictionaryCheckersPiecesFrom[maxCountedPiecesFrom] == 2
                    ) and (self.dictionaryCheckersPiecesTo[maxCountedPiecesTo] == 2):
                        self.notifyObserverViewCheckers(
                            message="a Move was found !"
                        )  # notify the view about the move of player

                        # verify if the user activate or disable the mandatory jumps in view
                        if self.levelCheckers == 1:  # if mandatory jumps activate
                            fromMove1, fromMove2, toMove1, toMove2, viaPiece, matrix = (
                                self.checkers.play(
                                    maxCountedPiecesFrom,
                                    maxCountedPiecesTo,
                                    mandatory_jumping=True,
                                )
                            )  # send the start position and cible position of moved player piece to minimax and alpha beta pruning algorithm
                        ...
                        self.matrix = np.array(
                            self.matrix
                        )  # convert the matrix list to numpy array

                        self.xy_coord_From = self.dictCheckers[
                            "{i}{j}".format(i=fromMove1, j=fromMove2)
                        ]  # save the pixel start position of the piece that has been moved by the computer into the variable self.xy_coord_From for appModel
                        self.xy_coord_To = self.dictCheckers[
                            "{i}{j}".format(i=toMove1, j=toMove2)
                        ]  # save the pixel cible position of the piece that has been moved by the computer into the variable self.xy_coord_To for appModel

                        if (
                            len(viaPiece) != []
                        ):  # if a player piece has been jumped by the computer
                            for i in viaPiece:
                                self.viaPiece.append(
                                    self.dictCheckers["{j}".format(j=i)]
                                )  # save the pixel coordinate of the position of the jumped piece into the array self.viaPiece for appModel
                    ...
                    if (
                        self.dictionaryCheckersPiecesFrom[maxCountedPiecesFrom] >= 2
                    ) or (self.dictionaryCheckersPiecesTo[maxCountedPiecesTo] >= 2):
                        self.dictionaryCheckersPiecesFrom[maxCountedPiecesFrom] = 0
                        self.dictionaryCheckersPiecesTo[maxCountedPiecesTo] = 0
                        self.dictionaryCheckersPiecesFrom = {}
                        self.dictionaryCheckersPiecesTo = {}
                    ...
                    logger.error(
                        "AppView: error by getting the start and cible position of the moved piece of player in getPlayerCheckers()-function"
                    )
    ...
    def getCentersFields(self, frame):
        ...
        _, frame_thresh = cv2.threshold(
            frame_gray, 170, 255, cv2.THRESH_BINARY_INV
        )  # convert gray frames to binary frames
        cnt = self.findBiggestContour(frame_thresh)
        self.x, self.y, h, w = cv2.boundingRect(
            cnt
        )  # get biggest contour (contour of gameboard)
        ...
        for num_column in range(0, 9):
            for num_line in range(0, 9):
                points.append(
                    ((num_line * line) + self.x, (num_column * column) + self.y)
                )
        ...
        if len(fields) != 0:
            for i in range(len(fields)):
                coord = (
                    int((fields[i][2] + fields[i][3]) / 2),
                    int((fields[i][0] + fields[i][1]) / 2),
                )
    ...
    def runCheckers_AutomaticallyDetection(self, frame):
        ...
        arr_centroids = (
            []
        )  # to save the middle positions of each fields (64 fields) of the game board
        fields = []
        frame_gray = cv2.cvtColor(
            frame, cv2.COLOR_BGR2GRAY
        )  # convert BGR frames to gray frames

        _, frame_thresh = cv2.threshold(
            frame_gray, 170, 255, cv2.THRESH_BINARY_INV
        )  # convert gray frames to binary frames
        cnt = self.findBiggestContour(
            frame_thresh
        )  # get biggest contour (contour of gameboard)
    ...
    def runCheckers_ManuallyDetection(self, frame):
        ...
        """
        frame_copy = frame.copy()
        arr_centroids = np.loadtxt(
            CENTERSCHECKERSFIELDS
        )  # load the saved centers pixel coordinates positions of all checker gameboard fields

        arr_centroids = tuple(map(tuple, arr_centroids))

        if len(arr_centroids) != 64:  # if the file is damaged
            self.notifyObserverViewCheckers(
                message="Please recalibrate the checkers board. Use the button `Calibrate Checkers Gameboard to get the center of fields`"
            )

        elif (
            len(arr_centroids) == 64
        ):  # if the file is not damaged, then send the 64 centers coordinates of all checkers fields with the RGB frame to the checker algorithm

            self.getPlayerCheckers(arr_centroids, frame_copy)
            for i in self.dictCheckers:
                cv2.circle(
                    frame,
                    (int(self.dictCheckers[i][0]), int(self.dictCheckers[i][1])),
                    3,
                    (0, 0, 255),
                    -1,
                )
    ...
    def startGameTicTT(self):
        ...
        self.notifyObserverViewTicTT(
            newScore=self.scoreTicTT
        )  # send the initial score to view
    ...
    def notifyObserverModel_TicTT_HomeView(self):
        ...
        """
        logger.debug(
            "AppController tic-tac-toe game: notify the model about the changed values..."
        )
        self.observerModel.updateTicTT_HomeView(
            cornersdataTicTT=self.cornersdataTicTT,
            stopFPSTicTT=self.stopFPSTicTT,
            newBrightness=self.brightness,
            stopFPSHomeView=self.stopFPSHomeView,
            cornersTableData=self.cornersTableData,
            center_coordPixel=self.center_coordPixel,
            appName=self.appName,
    ...
    def stopTicTT(self):
        ...
        """
        logger.debug(
            "AppController tic-tac-toe game: current tic-tac-toe game has been stopped!"
        )
    ...
    def notifyObserverViewTicTT(self, newScore=None, message=None):
        ...
        """
        logger.debug(
            "AppController tic-tac-toe game: notify view with current scores and notifications of the game process"
        )
    ...
    def updateTicTT(
        self, stopFPSTicTT=False, levelTicTT=1, cornersdataTicTT=None, appName=None
    ):
        """
    ...
    def getPlayer(self, points, frame):
        ...
        for field in points:
            cropped = self.getGameField(
                field, frame
            )  # croppes the frame with pixel coordinates in array points
            value = self.getShape(
                cropped
            )  # get the detected symbol on the cropped image
            old_played = np.where(indexs == gameboardIndex)[
                0
            ]  # get the index of the arrays where the 'gameboardIndex' is present.
            if value != False and self.initGame == False and len(old_played) == 1:
                self.notifyObserverViewTicTT(
                    message="A Move was found. Please wait 3 Seconds"
                )
            ...
            if (
                value != False
                and int(time.time()) - self.last_time >= 3
                and self.initGame == True
                and len(old_played) == 1
            ):
                self.notifyObserverViewTicTT(
                    message="Player chooses {}".format(gameboardIndex)
                )
    ...
    def resultReactionWIN_LOST(
        self,
        messageObserView1,
        playAudioFilename,
        messageObserView2,
        incrScorePlayer=False,
        incrScoreRobot=False,
    ):
        """
    ...
    def runticTT(self, frame):
        ...
        _, frame_thresh = cv2.threshold(
            frame_gray, 170, 255, cv2.THRESH_BINARY_INV
        )  # convert GRAY to binary images

        grid = self.get_board_template(
            frame_thresh
        )  # get the pixels coordinates of the tic tac toe game field

        for i, (x, y, w, h) in enumerate(
            grid
        ):  # draw rectangles aver the game field with the pixels coordinates
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 0), 2)

        if self.choose_user == True:  # if it is the player's turn
            result = self.getPlayer(
                grid, frame_thresh
            )  # check if the player has played. If yes get the index of the field, who the player has played and write this into the array of the game field for game algorithm. At the end, check if the player has won or not
            if result == True:  # verify if the player has won
                self.resultReactionWIN_LOST(
                    WIN_PLAYER, GAMEWIN, GAMEBOARDCLEAN, incrScorePlayer=True
                )  # play audio, notify the view about that the player has won the game and increment the score of player

            elif result == TIEGAME:  # verify if tie game
                self.resultReactionTIE_GAME(
                    messageObserView1=TIEGAME, messageObserView2=GAMEBOARDCLEAN
                )  # play audio, notify the view about that the Game is tie game

        elif self.choose_user == False:  # if it is the computer's turn
            self.robotAlgo(
                self.computer
            )  # the computer do a move with the tic tac toe algorithm and write O's Symbol in tic tac toe array. At the end it verify if the game is tie game or if the computer has won the game
            result = self.verifyWinner(
                self.computer
            )  # get the result of the computer move
            try:
            ...
            if result == True:  # verify if the computer has won
                self.resultReactionWIN_LOST(
                    WIN_FRANKA, GAMEOVER, GAMEBOARDCLEAN, incrScoreRobot=True
                )  # play audio, notify the view about that the computer has won the game and increment the score of computer
        ...
        arr = self.detectCircle(
            frame
        )  # perform the detection of the circle pieces for the robot and save their middles coordinates into a array arr

        if self.write_comp == True:

            if (
                self.computer == "O"
            ):  # this values will be used to write the moves of player and computer into gameboard virtually
                text = "O"
            else:
                text = "X"

            gameboard = np.array(self.board)
            indexs = np.where(gameboard == self.computer)[
                0
            ]  # get indexs of the gameboard array wo the computer has played
            for index in indexs:
                x, y, w, h = grid[
                    index
                ]  # get the rectangle pixels coordinates of index into the grid
                centroid = (
                    x + int(w / 2),
                    y + int(h / 2),
                )  # calculate the center of this region of interest
                cv2.circle(
                    frame, centroid, 3, (0, 255, 0), -1
                )  # draw the center of the circle pieces of computer virtually inside the gameboard

                if text == "O":
                    cv2.circle(
                        frame, centroid, 20, (0, 0, 255), 2
                    )  # draw the circle pieces of computer virtually inside the gameboard
            ...
            if len(indexs) == len(
                self.counterIndexTicTT
            ):  # if the arrays egals the helper variable of array, then pass
                pass
            else:  # if different
                arraysIndexTicTT = np.setdiff1d(
                    indexs, self.counterIndexTicTT
                )  # get the new index that differ on that two arrays
                ...
                if len(arr) == 0:  # if no pieces then notify the view
                    self.notifyObserverViewCheckers(
                        message="No circle piece has been detected. Please place the circle pieces in the view of camera so that the robot can pick it up"
                    )
                else:
                    ...
                    for (
                        ...
                        arr
                    ):  # only one piece of the detected circles pieces must be taken by the robot
                        ...
                        if (
                            self.rectContrains(
                                (grid[0][0], grid[0][1]),
                                (grid[0][0] + grid[0][2], grid[0][1] + grid[0][3]),
                                i,
                            )
                            == True
                            or self.rectContrains(
                                (grid[1][0], grid[1][1]),
                                (grid[1][0] + grid[1][2], grid[1][1] + grid[1][3]),
                                i,
                            )
                            == True
                            or self.rectContrains(
                                (grid[2][0], grid[2][1]),
                                (grid[2][0] + grid[2][2], grid[2][1] + grid[2][3]),
                                i,
                            )
                            == True
                            or self.rectContrains(
                                (grid[3][0], grid[3][1]),
                                (grid[3][0] + grid[3][2], grid[3][1] + grid[3][3]),
                                i,
                            )
                            == True
                            or self.rectContrains(
                                (grid[4][0], grid[4][1]),
                                (grid[4][0] + grid[4][2], grid[4][1] + grid[4][3]),
                                i,
                            )
                            == True
                            or self.rectContrains(
                                (grid[5][0], grid[5][1]),
                                (grid[5][0] + grid[5][2], grid[5][1] + grid[5][3]),
                                i,
                            )
                            == True
                            or self.rectContrains(
                                (grid[6][0], grid[6][1]),
                                (grid[6][0] + grid[6][2], grid[6][1] + grid[6][3]),
                                i,
                            )
                            == True
                            or self.rectContrains(
                                (grid[7][0], grid[7][1]),
                                (grid[7][0] + grid[7][2], grid[7][1] + grid[7][3]),
                                i,
                            )
                            == True
                            or self.rectContrains(
                                (grid[8][0], grid[8][1]),
                                (grid[8][0] + grid[8][2], grid[8][1] + grid[8][3]),
                                i,
                            )
                            == True
                        ):
                            ...
                            if self.stopNotifyObserverModel_TicTT_HomeView == False:
                                x, y, w, h = grid[
                                    arraysIndexTicTT[0]
                                ]  # get the coordinates of the new index, wo the computer has played
                                centroid = (
                                    x + int(w / 2),
                                    y + int(h / 2),
                ...
                self.counterIndexTicTT = (
                    indexs  # save the current indexs arrays in the helper array
                )

            # if the game has done, a new game will be started in 30 seconds
            if (
                self.autoStartTicTT == True
            ):  # 2 seconds after the game is finished, all messages to the model must be stopped.
                if int(time.time()) - self.last_time_TicTT >= 2:  #
                    self.stopNotifyObserverModel_TicTT_HomeView = True

                if (
                    int(time.time()) - self.last_time_TicTT >= 30
                ):  # 30 seconds after the game is finished, a new game can be start automatically.
                    self.initGameTicTT()
    ...
    def isWinner(self, board, letter):
        ...
        return (
            ...
            or (board[0] == letter and board[1] == letter and board[2] == letter)
    ...
    def run(self):
        ...
        while not self.event.is_set():
            ...
            if (
                not self.myResultFrameQueue_Model_Controller.empty()
            ):  # verify if the queue from appModel to appController is not empty
                self.frame = (
                    self.myResultFrameQueue_Model_Controller.get()
                )  # get the frames from the queue
                if (
                    self.gameName == "TicTT" and self.stopgameticTT == False
                ):  # if the user has started the game tic tac toe
                    try:
                        self.frame = self.runticTT(
                            self.frame
                        )  # the game can be started. The function runticTT use the frame for tic tac toe view from the appModel
                    except:
                        logger.error(
                            "appController: probleme with camera frame in function run () for tic tac toe game ..."
                        )

                if (
                    self.appName == "tictactoe" and self.gameName != "TicTT"
                ):  # if the current opened windows is tic tac toe view and the game tic tac toe has not started
                    arr = self.detectCircle(
                        self.frame
                    )  # then detect circle pieces for the robot
                    if (
                        len(arr) == 0
                    ):  # if none circle pieces has detected, then notify the view
                        self.notifyObserverViewTicTT(
                            message="No circle piece has been detected. Please place the circle pieces in the view of Camera so that the robot can pick it up"
                        )
                    else:
                ...
                if (
                    self.gameName == "Checkers" and self.stopgameCheckers == False
                ):  # if the user has started the checker game

                    if (
                        self.use_getCenterofFields == 1
                    ):  # if the user has clicked on the button "detection of the centers of the fields": "automatically" to start an automatic detection on gameboard and game pieces (the game board and game pieces can be moved in the camera area)
                ...
                ):  # if the user click on "Calibrate Checkers Gameboard to get the centers of fields", getCenterofFieldsCheckers is set to False and the detection of the centers of fields beginn.
                    self.frame = self.getCentersFields(self.frame)

                if (
                    self.getCenterofFieldsCheckers == True
                ):  # if the user click on "Get Centers now", the current detected centers of fields were be saved into './data/centers_Checkers_Fields.txt'
                    np.savetxt(CENTERSCHECKERSFIELDS, self.arr_centroids, fmt="%i")
                    self.notifyObserverViewCheckers(
                        message="the centers of fields has been saved ..."
                    )

            if (
                not self.myResultFrameQueue_Controller_Model.full()
            ):  # send the result frame into a queue to the appModel
