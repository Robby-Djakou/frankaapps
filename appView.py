...
class AppView(threading.Thread):
    def __init__(
        ...
        self.observer.registerObserverView(
            self
        )  # register appView as Observer of appController (observable) to get score of Checkers and Tic Tac Toe
        ...
        self.cornersTable = (
            []
        )  # hold 4 pixel coordinates points on the table for calibration matrix

        ############ Tower of Hanoi ###########
        self.dataTOH = (
            []
        )  # content the enter information by the user (from Pegs, to Pegs, temporyPegs), the Pixel coordinate of the 3 towers and the disc number
        ...
        self.scoreCheckers = [
            0,
            0,
        ]  # get current score (player, Franka Roboter) from appController for checkers

        ############ Tic Tac Toe ###############
        self.scoreTicTT = [
            0,
            0,
        ]  # get current score (player, Franka Roboter) from appController for Tic Tac Toe
        ...
        self.window.bind(
            "<KeyPress>", self.onKeyPressHomeView
        )  # bind main view with keypress event (:q to exit the view, :d to delete points during the calibration)
        self.window.bind(
            "<Escape>", self.onEscape
        )  # bind main view with keypress event escape to exit the view
        ...
        self.canvas = tkinter.Canvas(
            self.tkVideoFrame, width=VIEWWIDTH, height=VIEWHEIGHT
        )
        self.canvas.pack()
        ...
        self.labelText = Label(
            ...
            background="orange",
            anchor=CENTER,
            text="Please select corners or black points of robot table with the button `Calibrate Camera` and select one game !",
        ...
        self.labelText2 = Label(
            ...
            background="orange",
            anchor=CENTER,
            text="If the Position of the Camera since restart of application hasn't moved, you don't need to use `Calibrate Camera`",
            font=("verdana", 14, "bold"),
        )
        ...
        self.btnTicTacToe = Button(
            self.tkBtnFrame,
            text="Tic Tac Toe Game",
            font=("verdana", 18, "bold"),
            command=self.tictacToe,
        )
        self.btnTowerofHanoi = Button(
            self.tkBtnFrame,
            text="Tower of Hanoi",
            font=("verdana", 18, "bold"),
            command=self.towerofHanoi,
        )
        self.btnchessGame = Button(
            self.tkBtnFrame,
            text="Checkers Game",
            font=("verdana", 18, "bold"),
            command=self.checkers,
        )

        self.btnTicTacToe.pack(ipadx=5, ipady=15, fill="both", side="left", expand=True)
        self.btnTowerofHanoi.pack(
            ipadx=5, ipady=15, fill="both", side="left", expand=True
        )
        self.btnchessGame.pack(
            ipadx=5, ipady=15, fill="both", side="right", expand=True
        )
        ...
        self.btnCalibrate = Button(
            self.tkBtn2Frame,
            text="Calibrate Camera",
            borderwidth=3,
            command=self.findConersTable,
        )
        self.btnCalibrate.pack(
            ipadx=10, ipady=10, fill="both", anchor=tkinter.CENTER, expand=True
        )
    ...
    def playAudio(self, audiofilename):
        ...
        logger.debug(
            "AppView: audiofilename= {} will been played".format(audiofilename)
        )
        alert.play()
    ...
    def onKeyPressHomeView(self, key):
        ...
        if key.char == "q":  # to exit from view
            ...
            key.char == "d" and len(self.cornersTable) > 0
        ):  # to delete points during the calibration for zoom
            self.cornersTable.pop()
        elif (
            key.char == "d" and len(self.cornersTable) == 0
        ):  # to cancel the calibration process when it has already been started
            self.canvas.unbind("<Button 1>")  # unregister Click-Handler
            self.btnCalibrate.config(
                font=("verdana", 12, "normal"), fg="black", text="Calibrate Camera"
            )  # set the text of calibration button to origin
            self.getCORNERSTABLE = False
            self.allPointsCORNERS = (
                self.allPointsCORNERS_New
            )  # the copy matrix will been reuse, when the calibration will been canceled
    ...
    def notifyObserverHomeView(self):
        ...
        self.observer.updateHomeView(
            brightness=self.brightness,
            stopFPSHomeView=self.getCORNERSTABLE,
            cornersTableData=self.allPointsCORNERS,
    ...
    def findConersTable(self):
        ...
        if not self.getCORNERSTABLE:
            ...
            self.allPointsCORNERS_New = (
                self.allPointsCORNERS
            )  # copy the current matrix, if the user cancel the calibration, the old matrix can been reused
    ...
    def updateTK(self):
        ...
        if (
            self.getCORNERSTABLE and len(self.cornersTable) == 0
        ):  # to start new calibration
        ...
        if not self.resultFrameConsumer.empty():
            ...
            if not self.frame.size == 0:
                ...
                self.photo = PIL.ImageTk.PhotoImage(
                    master=self.canvas, image=PIL.Image.fromarray(self.frame)
                )
        ...
        if len(self.cornersTable) == 1:
            ...
            np.savetxt(
                CORNERSTABLE_DATA, self.allPointsCORNERS, fmt="%f"
            )  # save the current matrix after the opencv transformation
            not self.notifyObserverHomeView()
            self.cornersTable = (
                []
            )  # empty the cornersTable to reuse it for a new calibration
            logger.debug("AppView: calibration in Home View is finish")
        self.window.after(
            self.delay, self.updateTK
        )  # update the Tkinter GUI after 40 milliseconds
    ...
    def tictacToe(self):
        ...
        self.windowTicTT = Toplevel(
            self.window
        )  # initialize Tic Tac Toe View from Home View

        self.windowTicTT.title("Tic Tac Toe Game")  # set title of view
        self.windowTicTT.bind(
            "<KeyPress>", self.onKeyPressTicTT
        )  # bind tic tac toe view with keypress event (:q to exit the view, :d to delete points during the calibration)
        self.windowTicTT.bind(
            "<Escape>", self.onEscape
        )  # bind tic tac toe view with keypress event escape to exit the view
        ...
        self.canvasTicTT = tkinter.Canvas(
            tkVideoFrameTicTT, width=VIEWWIDTH, height=VIEWHEIGHT
        )
        ...
        self.notificationLabelTicTT = Label(
            tFrame0, font=("verdana", 14, "bold"), anchor=CENTER
        )
        self.notificationLabelTicTT.pack(
            ipadx=10, ipady=10, expand=True, fill="both", side="top"
        )
        ...
        labelRobot = Label(
            tkScoreFrame, text="Franka Roboter", font=("verdana", 24, "bold")
        )
        labelRobot.grid(row=0, column=2)
        self.labelPlayerScoreTicTT = Label(
            tkScoreFrame, text="-", font=("verdana", 48, "bold")
        )
        self.labelPlayerScoreTicTT.grid(row=1, column=0, padx=100)
        labelPlayerSeparator = Label(
            tkScoreFrame, text=":", font=("verdana", 48, "bold")
        )
        labelPlayerSeparator.grid(row=1, column=1, padx=20)
        self.labelRobotScoreTicTT = Label(
            tkScoreFrame, text="-", font=("verdana", 48, "bold")
        )
        ...
        radioBtnLevel1 = Radiobutton(
            tkBtnFrame,
            variable=self.levelTicTT,
            value=0,
            indicatoron=0,
            borderwidth=3,
            font=("verdana", 12, "bold"),
        ...
        self.btnStartTicTT = Button(
            tkBtn1Frame,
            text="New Game",
            font=("verdana", 18, "bold"),
            activebackground="green",
            borderwidth=3,
            command=self.newGameTicTT,
        )
        self.btnStartTicTT.pack(
            ipadx=5, ipady=10, fill="both", side="left", expand=True
        )
        ...
        self.btnCalibrateTicTT = Button(
            tkBtn2Frame,
            text="Calibrate Tic Tac Toe Gameboard for Zoom",
            width=50,
            borderwidth=3,
            command=self.findConersTicTT,
        )
        self.btnCalibrateTicTT.pack(
            ipadx=10, ipady=10, fill="both", anchor=tkinter.CENTER, expand=True
        )

        self.cornersTicTT = (
            []
        )  # hold 4 pixel coordinates points for calibration matrix (zoom on tic tac toe gameboard)
    ...
    def onKeyPressTicTT(self, key):
        ...
        if key.char == "q":
            ...
            self.btnCalibrateTicTT.config(
                font=("verdana", 12, "normal"),
                fg="black",
                text="Calibrate Tic Tac Toe Gameboard",
            )
    ...
    def notifyObserverTicTT(self):
        ...
        self.observer.updateTicTT(
            stopFPSTicTT=self.getCORNERSTicTT,
            levelTicTT=self.levelTicTT.get(),
            cornersdataTicTT=self.allPoints,
            appName=self.appName,
        )
    ...
    def updateTKTicTT(self):
        ...
        if (
            self.getCORNERSTicTT and len(self.cornersTicTT) == 0
        ):  # to start new calibration
        ...
        if not self.resultFrameConsumer_Game.empty():
            ...
            if not self.frameTicTT.size == 0:
                ...
                self.photoTicTT = PIL.ImageTk.PhotoImage(
                    master=self.canvasTicTT, image=PIL.Image.fromarray(self.frameTicTT)
                )
                self.canvasTicTT.create_image(
                    0, 0, image=self.photoTicTT, anchor=tkinter.NW
                )  # draw camera frame on canvas
        ...
        if len(self.cornersTicTT) == 1:
            ...
            np.savetxt(
                CORNERSTicTT, self.allPoints, fmt="%f"
            )  # save the current matrix after the opencv transformation
        ...
        self.windowTicTT.after(
            self.delay, self.updateTKTicTT
        )  # update the Tkinter GUI after 100 milliseconds
    ...
    def checkers(self):
        ...
        self.windowCheckers = Toplevel(
            self.window
        )  # initialize checkers View from Home View
        self.windowCheckers.title("Checkers Game")  # set title of view
        self.windowCheckers.bind(
            "<KeyPress>", self.onKeyPressCheckers
        )  # bind checkers view with keypress event (:q to exit the view, :d to delete points during the calibration)
        self.windowCheckers.bind(
            "<Escape>", self.onEscape
        )  # bind checkers view with keypress event escape to exit the view
        ...
        self.canvasCheckers = tkinter.Canvas(
            tkVideoFrameCheckers, width=VIEWWIDTH, height=VIEWHEIGHT
        )
        ...
        tFrame0.pack(fill=BOTH, expand=True)
        self.notificationLabelCheckers = Label(
            tFrame0, font=("verdana", 14, "bold"), anchor=CENTER
        )
        self.notificationLabelCheckers.pack(
            ipadx=10, ipady=10, expand=True, fill="both", side="top"
        )
        ...
        labelRobot.grid(row=0, column=2)
        self.labelPlayerScoreCheckers = Label(
            tkScoreFrame, text="-", font=("verdana", 48, "bold")
        )
        ...
        labelPlayerSeparator.grid(row=1, column=1, padx=20)
        self.labelRobotScoreCheckers = Label(
            tkScoreFrame, text="-", font=("verdana", 48, "bold")
        )
        ...
        radioBtnLevel1 = Radiobutton(
            tkBtnFrame,
            variable=self.levelCheckers,
            value=0,
            indicatoron=0,
            borderwidth=3,
            font=("verdana", 12, "bold"),
            text="disable mandatory jump",
            width=10,
            command=self.notifyObserverCheckers,
        )
        radioBtnLevel1.pack(ipadx=10, ipady=10, expand=True, fill="both", side="left")
        ...
        labe_useButton_get_center = Label(
            tkBtnFrame_get_center,
            text="Detection of the centers of the fields:",
            font=("verdana", 16, "bold"),
        )
        labe_useButton_get_center.pack(
            padx=20, ipady=10, expand=True, fill="both", side="left"
        )

        # radiobutton to deactivate automaticaly detection of centers of the fields
        radioBtnEnableDetection = Radiobutton(
            tkBtnFrame_get_center,
            variable=self.useButton_get_center,
            value=0,
            indicatoron=0,
            borderwidth=3,
            font=("verdana", 12, "bold"),
        ...
        radioBtnEnableDetection.pack(
            ipadx=10, ipady=10, expand=True, fill="both", side="left"
        )

        # radiobutton to activate automaticaly detection of centers of the fields
        radioBtnDisableDetection = Radiobutton(
            tkBtnFrame_get_center,
            variable=self.useButton_get_center,
            value=1,
            indicatoron=0,
        ...
        radioBtnDisableDetection.pack(
            ipadx=10, ipady=10, expand=True, fill="both", side="left"
        )

        logger.debug(
            "Detection of the centers of the fields {}".format(
                self.useButton_get_center.get()
            )
        )
        ...
        self.btnStartCheckers = Button(
            tkBtn1Frame,
            text="New Game",
            font=("verdana", 18, "bold"),
            activebackground="green",
            borderwidth=3,
            command=self.newGameCheckers,
        )
        self.btnStartCheckers.pack(
            ipadx=5, ipady=10, fill="both", side="left", expand=True
        )

        tkBtnpause_continueFrame = Frame(
            self.windowCheckers, relief=RIDGE, borderwidth=1
        )
        tkBtnpause_continueFrame.pack(fill=BOTH, expand=True)

        self.btncontinueCheckers = Button(
            tkBtnpause_continueFrame,
            text="Continue the last saved checkers game status",
            font=("verdana", 18, "bold"),
            activebackground="green",
            borderwidth=3,
            command=self.continueGameCheckers,
        )
        self.btncontinueCheckers.pack(
            ipadx=5, ipady=10, fill="both", side="right", expand=True
        )

        self.btnpauseCheckers = Button(
            tkBtnpause_continueFrame,
            text="Save current checkers game status",
            font=("verdana", 18, "bold"),
            activebackground="#ff8000",
            borderwidth=3,
            command=self.saveStateGameCheckers,
        )
        self.btnpauseCheckers.pack(
            ipadx=5, ipady=10, fill="both", side="right", expand=True
        )
        ...
        btnQuitCkeckers = Button(
            ...
            width=50,
            borderwidth=3,
            command=self.exitCheckers,
        )
        btnQuitCkeckers.pack(
            ipadx=10, ipady=10, fill="both", side="bottom", expand=True
        )
        ...
        self.btnCalibrateCheckers = Button(
            tkBtn2Frame,
            text="Calibrate Checkers Gameboard for Zoom",
            width=50,
            borderwidth=3,
            command=self.findConersCheckers,
        )
        self.btnCalibrateCheckers.pack(
            ipadx=10,
            ipady=10,
            fill="both",
            side="left",
        ...
        self.btnCalibrateCheckers2 = Button(
            tkBtn2Frame,
            text="Calibrate Checkers Gameboard to get the centers of fields",
            width=50,
            borderwidth=3,
            command=self.findCenterFieldsCheckers,
        )
        self.btnCalibrateCheckers2.pack(
            ipadx=10,
            ipady=10,
            fill="both",
            side="right",
        ...
        self.cornersCheckers = (
            []
        )  # hold 4 pixel coordinates points for calibration matrix
    ...
    def onKeyPressCheckers(self, key):
        ...
        if key.char == "q":  # to exit from view
            ...
            key.char == "d" and len(self.cornersCheckers) > 0
        ):  # to delete points during the calibration for zoom
            ...
            self.btnCalibrateCheckers.config(
                font=("verdana", 12, "normal"),
                fg="black",
                text="Calibrate Checkers Gameboard",
            )  # set the text of calibration button to origin
            self.getCORNERSCHECKERS = False
            self.allPointsCheckers = (
                self.allPointsCheckers_new
            )  # the copy matrix will been reuse, when the calibration will been canceled
    ...
    def notifyObserverCheckers(self):
        ...
        self.observer.updateCheckers(
            stopFPSCheckers=self.getCORNERSCHECKERS,
            levelCheckers=self.levelCheckers.get(),
            cornersdataCheckers=self.allPointsCheckers,
            appName=self.appName,
            getCenterofFieldsCheckers=self.getCenterofFieldsCheckers,
            use_getCenterofFields=self.useButton_get_center.get(),
        )
    ...
    def continueGameCheckers(self):
        ...
        if os.path.isfile(SAVE_STATE_CHECKERS_GAME):
            ...
            if len(arr) != 0:
                ...
                self.updateCheckersView(
                    message="Please delete manually the file `continue_pause.npy` in Folder `data`"
                )
    ...
    def newGameCheckers(self):
        ...
        if self.useButton_get_center.get() == 1:
            ...
            if os.path.isfile(CENTERSCHECKERSFIELDS):
                ...
                self.updateCheckersView(
                    message="Error ! Cannot find the data {}. Please use the button `Calibrate Checkers Gameboard to get the center of fields`".format(
    ...
    def findCenterFieldsCheckers(self):
        ...
        if self.getcalibrateCentersFieldsCheckers == False:
            self.notificationLabelCheckers.config(
                text="When you don't see the centers of the fields, you can move the gameboard to a different position. Click on 'Get Centers now' when you see the centers of the 64 fields",
                background="red",
            )
            self.btnCalibrateCheckers2.config(
                font=("verdana", 12, "bold"), fg="red", text="Get Centers now"
            )
    ...
    def drawCornersAndGridCheckers(self):
        ...
        if self.getCORNERSCHECKERS:
            ...
            for i in range(1, steps):
                ...
                ymax -= deltaY
                cv2.rectangle(
                    self.frameCheckers, (xmin, ymin), (xmax, ymax), (0, 0, 0), 1
                )
    ...
    def updateTKCheckers(self):
        ...
        if (
            self.getCORNERSCHECKERS and len(self.cornersCheckers) == 0
        ):  # to start new calibration
        ...
        if not self.resultFrameConsumer_Game.empty():
            ...
            if not self.frameCheckers.size == 0:
                ...
                self.photoCheckers = PIL.ImageTk.PhotoImage(
                    master=self.canvasCheckers,
                    image=PIL.Image.fromarray(self.frameCheckers),
                )
                self.canvasCheckers.create_image(
                    0, 0, image=self.photoCheckers, anchor=tkinter.NW
                )  # draw camera frame on canvas
        ...
        if len(self.cornersCheckers) == 1:
            ...
            np.savetxt(
                CORNERSCHECKERS, self.allPointsCheckers, fmt="%f"
            )  # save the current matrix after the opencv transformation
        ...
        self.windowCheckers.after(
            self.delay_games_windows, self.updateTKCheckers
        )  # update the Tkinter GUI after 100 milliseconds
    ...
    def towerofHanoi(self):
        ...
        self.windowTOH = Toplevel(
            self.window
        )  # initialize tower of hanoi View from Home View
        self.windowTOH.title("Tower of Hanoi Game")  # set title of view
        self.windowTOH.bind(
            "<KeyPress>", self.onKeyPressTOH
        )  # bind tower of hanoi view with keypress event (:q to exit the view, :d to delete points during the calibration)
        self.windowTOH.bind(
            "<Escape>", self.onEscape
        )  # bind tower of hanoi view with keypress event escape to exit the view
        ...
        headTOH = Label(
            tFrameTitle,
            anchor=CENTER,
            text="Welcome to Tower of Hanoi Game",
            font=("verdana", 20, "bold"),
        )
        ...
        self.canvasTOH = tkinter.Canvas(
            tkVideoFrameTOH, width=VIEWWIDTH, height=VIEWHEIGHT
        )
        self.canvasTOH.pack()
        ...
        self.notificationLabelTOH.pack(
            ipadx=10, ipady=10, expand=True, fill="both", side="top"
        )
        ...
        textfield1 = Label(
            tFrame1, text="Please enter the number of disc: ", font=("verdana", 16)
        )
        ...
        textfield12 = Label(
            tFrame12, text="From pegs (A or B or C):", font=("verdana", 16)
        )
        ...
        textfield121 = Label(
            tFrame121, text="To Pegs (A or B or C):", font=("verdana", 16)
        )
        ...
        textfield2 = Label(
            tFrame2,
            text="Please use the Button 'Select Tower' and select the top of Tower",
            font=("verdana", 16),
        )
        ...
        btnConfirmTOH = Button(
            ...
            width=50,
            borderwidth=3,
            command=self.verifyInput_and_start_TOH,
        )
    ...
    def selectTopTower(self):
        ...
        self.notificationLabelTOH.config(
            background="orange",
    ...
    def verifyInput_and_start_TOH(self):
        ...
        if (
            discnumber.isdigit() == True
            and self.storageTOH is not None
            and fromPegs in ("A", "B", "C")
            and toPegs in ("A", "B", "C")
        ):
            if fromPegs == toPegs:
                logger.error(
                    "AppView: error. the entry the values for tower of hanoi game is incorrect"
                )
                self.playAudio(BEEPFILE_ERROR)
                self.notificationLabelTOH.config(
                    background="red",
                ...
                temporaryPegs = set(ABC).difference(
                    set(setPegs)
                )  # get temporary pegs from the entry (from and to pegs)
                ...
                logger.debug(
                    "AppView: entry values is correct. Tower of Hanoi game will starting ..."
                )
        else:
            logger.error(
                "AppView: error. the entry the values for tower of hanoi game is incorrect"
    ...
    def updateTKTOH(self):
        ...
        if self.event.is_set():
            ...
            self.exit()
        if (
            not self.stopselectTower and len(self.counttower) == 0
        ):  # to start the functionality to get the pixel coordinates of tower
            self.selectTopTower()
            ...
            for x, y in self.counttower:
                ...
                if len(self.counttower) == 1:
                    ...
                    cv2.putText(
                        self.frame,
                        "A",
                        (self.counttower[0][0] + 20, self.counttower[0][1] + 10),
                    ...
                    cv2.putText(
                        self.frame,
                        "B",
                        (self.counttower[1][0] + 20, self.counttower[1][1] + 10),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        1,
                        (0, 255, 0),
                        3,
                    ...
                    cv2.putText(
                        self.frame,
                        "C",
                        (self.counttower[2][0] + 20, self.counttower[2][1] + 10),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        1,
                        (255, 110, 140),
                        3,
                    )
            self.photoTOH = PIL.ImageTk.PhotoImage(
                master=self.canvasTOH, image=PIL.Image.fromarray(self.frame)
            )
        ...
        if len(self.counttower) == 1:
            ...
            self.counttower = []
            logger.debug(
                "AppView: all pixel coordinates of pegs has been saved for tower of hanoi"
            )
        self.windowTOH.after(
            self.delay_games_windows, self.updateTKTOH
        )  # update the Tkinter GUI after 100 milliseconds
    ...
    def updateTicTTView(self, newScoreTicTT=None, message=None):
        ...
        if not message is None:
            try:
                self.notificationLabelTicTT.config(background="orange", text=message)
            except:
    ...
    def updateCheckersView(self, newScoreCheckers=None, message=None):
        ...
        if not message is None:  ##############################
            try:
                self.notificationLabelCheckers.config(background="orange", text=message)
            except:
