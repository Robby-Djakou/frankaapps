...
class AppModel(threading.Thread):
    def __init__(
        ...
        self.myResultFrameQueue_Controller_Model = (
            myResultFrameQueue_Controller_Model  # get frames from controller
        )
        ...
        self.stopFPSHomeView = (
            False  # stop fps during the calibration of camera view in homeview
        )
        self.cornersTableData = (
            None  # opencv matrix of 4 points on the white table of panda-robot
        )
        ...
        ################# Tic Tac Toe
        self.stopFPSTicTT = (
            False  # stop fps during the calibration of camera view in tic tac toe view
        )
        self.Z_object_TicTT = 0.027430  # panda z-coordinate of the tic tac toe circles piece, to grab this on a plane surface

        ################# Checker
        self.stopFPSCheckers = (
            False  # stop fps during the calibration of camera view in checker view
        )
        self.Z_object_Checkers = (
            0.023  # panda z-coordinate of hanoi piece, to grab this on a plane surface
        )
    ...
    def pixelToXY_Robot(
        ...
        CAMERA_REFERENCE = np.array([[centerx], [centery], [0.0], [1]])

        # Create the homogeneous transformation matrix from robot base to camera base
        HOMOGEN_TRANSFORMATION = np.concatenate(
            (ROTATION_ROBOT_CAMERA, TRANSLATION_VECTOR), axis=1
        )
    ...
    def updateTicTT_HomeView(
        self,
        cornersdataTicTT=None,
        newBrightness=0,
        stopFPSHomeView=False,
        stopFPSTicTT=False,
        cornersTableData=None,
        center_coordPixel=None,
        appName=None,
        circleData=None,
    ):
        """
        ...
        self.camStreamer.setBrightness(
            newBrightness
        )  # set the Brightness to VideoStreamer
        self.stopFPSHomeView = stopFPSHomeView
        self.stopFPSTicTT = stopFPSTicTT

        if (
            not circleData is None and not center_coordPixel is None
        ):  # if circle pieces for the panda-roboter detected and centers coordinates of tic tac toe gameboard fields, where the robot played is available
    ...
    def game_TicTT(self, center_coordPixel, circleData):
        ...
        robotX1, robotY1 = self.pixelToXY_Robot(
            circleData[0], circleData[1], X_ROBOT_SHIFFTING=-0.025
        )  # convert pixel coordinate of detected circle piece to panda coordinate
        self.robot.move(
            LinearMotion(Affine(robotX1, robotY1, 0.01556))
        )  # panda-robot use the result coordinate to go on the position of the circle piece
        self.gripper.clamp()  # Panda-robot close gripper to grab the circle piece
        self.robot.move(
            LinearMotion(Affine(robotX1, robotY1, 0.1))
        )  # panda-robot goes up one centimetre
        robotX2, robotY2 = self.pixelToXY_Robot(
            center_coordPixel[0], center_coordPixel[1]
        )  # convert the destination pixel coordinate to panda-coordinate
        self.robot.move(
            LinearMotion(Affine(robotX2, robotY2, 0.025))
        )  # panda-robot use the result  coordinate to go on the position of the destination field of tic tac toe gameboard
    ...
    def game_TOH(self):
        ...
        count_Z1, count_Z2, count_Z3 = (
            0,
            0,
            0,
        )  # Initialize the three tower at the beginning with no disc (0)

        for i in self.stateTOH:
            if (
                i[1] == 0
            ):  # if the start tower is 0 (A) where the discs have been placed.
                self.positionFrom = self.dataTOH[
                    4
                ]  # get value of tower A, where the discs have been placed.
                if (
                    count_Z1 == 0 and count_Z2 == 0 and count_Z3 == 0
                ):  # if tower A is  at the beginning of the game the start tower, the other 3 tower must be empty
                    count_Z1 = self.dataTOH[
                        0
                    ]  # tower A get the total number of the disc at the biginning
            ...
            if (
                i[1] == 1
            ):  # if the start tower is 1 (B) where the discs have been placed.
                self.positionFrom = self.dataTOH[
                    5
                ]  # get value of tower B, where the discs have been placed.

                if (
                    count_Z1 == 0 and count_Z2 == 0 and count_Z3 == 0
                ):  # if tower B is  at the beginning of the game the start tower, the other 3 tower must be empty
                    count_Z2 = self.dataTOH[
                        0
                    ]  # tower B get the total number of the disc at the biginning
            ...
            if (
                i[1] == 2
            ):  # if the start tower is 2 (C) where the discs have been placed.
                self.positionFrom = self.dataTOH[
                    6
                ]  # get value of tower C, where the discs have been placed.

                if (
                    count_Z1 == 0 and count_Z2 == 0 and count_Z3 == 0
                ):  # if tower C is  at the beginning of the game the start tower, the other 3 tower must be empty
                    count_Z3 = self.dataTOH[
                        0
                    ]  # tower C get the total number of the disc at the biginning
            ...
            if i[2] == 0:  # if the cible tower is 0 (A) where the discs will be placed.
                self.positionTo = self.dataTOH[
                    4
                ]  # pixel coordinate of destination tower if tower A
            ...
            if i[2] == 1:  # if the cible tower is 1 (B) where the discs will be placed.
                self.positionTo = self.dataTOH[
                    5
                ]  # pixel coordinate of destination tower if tower B
            ...
            if i[2] == 2:  # if the cible tower is 2 (C) where the discs will be placed.
                self.positionTo = self.dataTOH[
                    6
                ]  # pixel coordinate of destination tower if tower C
            ...
            coordinatesFrom = self.pixelToXY_Robot(
                self.positionFrom[0], self.positionFrom[1], X_ROBOT_SHIFFTING=0
            )  # convert start pixel position of tower into panda coordinate
            self.robot.move(
                LinearMotion(Affine(coordinatesFrom[0], coordinatesFrom[1], 0.1))
            )  # panda-robot use the result  coordinate to go on the start position

            self.robot.move(
                LinearMotion(Affine(coordinatesFrom[0], coordinatesFrom[1], Z_))
            )  # panda-robot use the current z calculated coordinate to grab the disc at the same position
            self.gripper.clamp()
            self.robot.move(
                LinearMotion(Affine(coordinatesFrom[0], coordinatesFrom[1], 0.1))
            )  # panda-robot goes up one centimetre

            coordinatesTo = self.pixelToXY_Robot(
                self.positionTo[0], self.positionTo[1], X_ROBOT_SHIFFTING=0
            )  # convert destination pixel position of tower into panda coordinate
            self.robot.move(
                LinearMotion(Affine(coordinatesTo[0], coordinatesTo[1], Z_ + 0.02))
            )  # panda-robot use the current z calculated coordinate to drop the disc 2 cm higher
            self.gripper.open()  # open gripper
            self.robot.move(
                LinearMotion(Affine(coordinatesTo[0], coordinatesTo[1], 0.1))
            )

            if count == self.counter_TOH:
                self.robot.move(
                    JointMotion(HOME_JOINT)
                )  # take home pose if total move is egal to count
            count += 1

    def update_Checkers(
        self, cornersdataCheckers=None, appName=None, stopFPSCheckers=False
    ):
        """
        update values using by the appController to notify the appModel.
    ...
    def update_Checkers_Robot(self, xy_coord_From, xy_coord_To, viaPiece):
        ...
        """
        self.game_Checkers(
            xy_coord_From, xy_coord_To, xy_coord_via=viaPiece
        )  # use the function game_Checkers to convert pixel into panda-koordinate and send its to panda-robot
    ...
    def game_Checkers(self, xy_coord_From, xy_coord_To, xy_coord_via=[]):
        ...
        robotFromX, robotFromY = self.pixelToXY_Robot(
            xy_coord_From[0], xy_coord_From[1]
        )  # convert pixel start position of the moved piece into panda coordinate
        robotToX, robotToY = self.pixelToXY_Robot(
            xy_coord_To[0], xy_coord_To[1]
        )  # convert pixel destination position of the moved piece into panda coordinate

        self.robot.move(LinearMotion(Affine(robotFromX, robotFromY, 0.1)))
        self.robot.move(
            LinearMotion(Affine(robotFromX, robotFromY, self.Z_object_Checkers))
        )  # panda robot move to this position to take the checker piece
        self.gripper.clamp()
        self.robot.move(LinearMotion(Affine(robotFromX, robotFromY, 0.1)))
        self.robot.move(LinearMotion(Affine(robotToX, robotToY, 0.1)))
        self.robot.move(
            LinearMotion(Affine(robotToX, robotToY, self.Z_object_Checkers))
        )  # panda robot move to this position to drop the gripped checker piece
        ...
        if len(xy_coord_via) != 0:  # if a player has been jumbed by computer piece
            for i in xy_coord_via:
                ...
                self.robot.move(
                    LinearMotion(Affine(robotViaX, robotViaY, self.Z_object_Checkers))
                )  # go to the position of player piece
                self.gripper.clamp()  # grab player piece
                self.robot.move(LinearMotion(Affine(robotViaX, robotViaY, 0.1)))
                self.robot.move(
                    LinearMotion(Affine(0.119622, -0.340037, 0.1))
                )  # drop player piece
                self.gripper.open()
            ...
            if self.frame is None or self.frame.size == 0:
                ...
                if self.cornersTableData is not None:
                    self.frame = cv2.warpPerspective(
                        self.frame, self.cornersTableData, (WIDTH, HEIGHT)
                    )  # opencv transformation of the 4 points on the robot table

                if FLIPHORIZONTAL == True:  # flip horizontal if activated
                    self.frame = cv2.flip(self.frame, 1)

                self.frame_Game = self.frame.copy()
                self.frame_Game_final = (
                    None  # result frame for tic tac toe and checkers view
                )

                if (
                    self.appName == "tictactoe"
                ):  # if the user has opened the tic tac toe window (view)
                    self.newFrameTime = time.time()  # get current time
                    if not self.myResultFrameQueue_Model_Controller.full():
                        self.myResultFrameQueue_Model_Controller.put(
                            self.frame_Game
                        )  # if queue empty, then send the frames to the controller for tic tac toe algorithm

                    if not self.myResultFrameQueue_Controller_Model.empty():
                        self.frame_Game_final = (
                            self.myResultFrameQueue_Controller_Model.get()
                        )  # if queue not empty, then get the result frames from the controller

                    if (
                        OPENCV_DEBUG_TIC_TAC_TOE == True
                    ):  # for debugging tic tac toe camera view
                    ...
                    if self.frame_Game_final is None or self.frame_Game_final.size == 0:
                        ...
                        if self.cornersdataTicTT is not None:
                            self.frame_Game_final = cv2.warpPerspective(
                                self.frame_Game_final,
                                self.cornersdataTicTT,
                                (WIDTH, HEIGHT),
                            )  # opencv transformation for zoom

                    if SHOWFPS == True and self.stopFPSTicTT == False:
                        outText = "FPS: {:.1f}".format(
                            1 / (self.newFrameTime - self.previousFrameTime)
                        )
                        cv2.putText(
                            self.frame_Game_final,
                            outText,
                    ...
                    if not self.myResultFrameQueue_Model_View.full():
                        if self.frame_Game_final is None:
                            ...
                            self.myResultFrameQueue_Model_View.put(
                                self.frame_Game_final
                            )  # send the result frames to the appView

                elif (
                    self.appName == "checkers"
                ):  # if the user has opened the checkers window (view)
                ...
                ):  # if the user has opened the homeview or tower of hanoi window (view)
                    self.newFrameTime = time.time()
                    ...
                    if not self.resultFrameProducer.full():
                        self.resultFrameProducer.put(
                            self.frame
                        )  # send frame to appView for tower of hanoi and homeview
                time.sleep(0.1)
