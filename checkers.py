...
class Node:
    ...
    def get_children(self, maximizing_player, mandatory_jumping):
        ...
        children_states = (
            []
        )  # array to save the next status of Node class (children of current Node class)
        big_letter = ""
        queen_row = 0
        if (
            maximizing_player is True
        ):  # if maximizing player, then find availables moves or possibles jumps for the computer
            available_moves = Checkers.find_available_moves(
                current_state, mandatory_jumping
            )  # verify if mandatory jump is True, then get available jumps or moves
        ...
        for i in range(len(available_moves)):
            ...
            Checkers.make_a_move(
                state, old_i, old_j, new_i, new_j, big_letter, queen_row
            )
...
class Checkers:
    ...
    def __init__(self, observerController, matrix):
        ...
        self.matrix = (
            matrix.tolist()
        )  # convert to numpy array to list array for the algorithm
        ...
        self.mandatory_jumping = (
            False  # initialize the mandatoring jumpy to False for 2 players
        )
        ...
        self.final_score = [
            0,
            0,
        ]  # initialize the score of the player and the score of computer
    ...
    def get_player_input(self, fromInd, toInd):
        ...
        )  # get the available moves or jumps from the current array gameboard of te player
        if (
            len(available_moves) == 0
        ):  # if the not available moves, then verify the number of pieces of both player
            if (
                self.computer_pieces > self.player_pieces
            ):  # if the computer has more piece than the player, then the computer won the game
                ...
                self.observerController.notifyObserverViewCheckers(
                    newScore=self.final_score, message=self.message
                )  # notify the appController with the score informations of both players and the notification message
        ...
        old = coord1.split(
            ","
        )  # get the x and y coordinates of the start position of the piece, that the player has moved
        new = coord2.split(
            ","
        )  # get the x and y coordinates of the cible position where the piece will be moved
        ...
        move = [
            ...
            int(new_i),
            int(new_j),
        ]  # do a array to save the x,y start position of player piece and x,y cible position of this piece

        # # find the player move in the available moves or available jumps
        if (
            move not in available_moves
        ):  # if the player move is not in the available jumps or moves
            ...
            self.message = (
                "Player has moved black piece from ({},{}) to ({},{})".format(
            ...
            Checkers.make_a_move(
                self.matrix, int(old_i), int(old_j), int(new_i), int(new_j), "B", 0
            )  # update the current gameboard after a jump or after a move of the player and verify if the player become king
            for m in range(8):
                for n in range(8):
                    if (
                        self.matrix[m][n][0] == "c" or self.matrix[m][n][0] == "C"
                    ):  # write the position changes of the computer pieces into the array of gameboard
                        self.computer_pieces += 1  # count number of computer pieces
                    elif (
                        self.matrix[m][n][0] == "b" or self.matrix[m][n][0] == "B"
                    ):  # write the position changes of the player pieces into the array of gameboard
    ...
    def find_available_moves(board, mandatory_jumping):
        ...
        for m in range(8):
            for n in range(8):
                if (
                    board[m][n][0] == "c"
                ):  # verify if the value of the array beginnt with 'c' (computer)
                    if Checkers.check_moves(
                        board, m, n, m + 1, n + 1
                    ):  # verify if the current move is authorized (cible position is bottom right from current position)
                        available_moves.append(
                            [m, n, m + 1, n + 1]
                        )  # if the current move is authorized then save this move into the available moves
                    if Checkers.check_moves(
                        board, m, n, m + 1, n - 1
                    ):  # verify if the current move is authorized (cible position is bottom left from current position)
                        available_moves.append([m, n, m + 1, n - 1])
                    if Checkers.check_jumps(
                        board, m, n, m + 1, n - 1, m + 2, n - 2
                    ):  # jump the computer piece over the player piece (cible position is bottom left from current position)
                        available_jumps.append([m, n, m + 2, n - 2])
                    if Checkers.check_jumps(
                        board, m, n, m + 1, n + 1, m + 2, n + 2
                    ):  # jump the computer piece over the player piece (cible position is bottom right from current position)
                        available_jumps.append([m, n, m + 2, n + 2])
                elif (
                    board[m][n][0] == "C"
                ):  # verify if the value of the array beginnt with 'C' (computer has becommed king)
                    if Checkers.check_moves(
                        board, m, n, m + 1, n + 1
                    ):  # verify if the current move is authorized (cible position is bottom right from current position)
                        available_moves.append(
                    ...
                    if Checkers.check_moves(
                        board, m, n, m - 1, n - 1
                    ):  # verify if the current move is authorized (cible position is top left from current position)
                        available_moves.append([m, n, m - 1, n - 1])
                    if Checkers.check_moves(
                        board, m, n, m - 1, n + 1
                    ):  # verify if the current move is authorized (cible position is top right from current position)
                        available_moves.append([m, n, m - 1, n + 1])
                    if Checkers.check_jumps(
                        board, m, n, m + 1, n - 1, m + 2, n - 2
                    ):  # jump the computer piece over the player piece (cible position is bottom left from current position)
                    ...
                    if Checkers.check_jumps(
                        board, m, n, m - 1, n - 1, m - 2, n - 2
                    ):  # jump the computer piece over the player piece (cible position is top left from current position)
                        available_jumps.append([m, n, m - 2, n - 2])
                    if Checkers.check_jumps(
                        board, m, n, m - 1, n + 1, m - 2, n + 2
                    ):  # jump the computer piece over the player piece (cible position is top right from current position)
                        available_jumps.append([m, n, m - 2, n + 2])
                    if Checkers.check_jumps(
                        board, m, n, m + 1, n + 1, m + 2, n + 2
                    ):  # jump the computer piece over the player piece (cible position is bottom right from current position)
        ...
        if mandatory_jumping is False:  # if mandatory jumping is not activated
            available_jumps.extend(
                available_moves
            )  # add elements of  available_moves to available_jumps. the computer can do a jump or a move
            return available_jumps
        elif mandatory_jumping is True:  # if mandatory jumping is activated
            if (
                len(available_jumps) == 0
            ):  # if not jump over player available, then use the computer must play his move
    ...
    def check_jumps(board, old_i, old_j, via_i, via_j, new_i, new_j):
        ...
        if (
            board[via_i][via_j] == "---"
        ):  # if the position of the piece, that has been jumped is empty
            return False
        if (
            board[via_i][via_j][0] == "C" or board[via_i][via_j][0] == "c"
        ):  # if the piece, that has been jumped is for the computer
            return False
        if (
            board[new_i][new_j] != "---"
        ):  # if the cible position of the computer piece is not empty
            return False
        if (
            board[old_i][old_j] == "---"
        ):  # if the start position of the computer piece is empty
            return False
        if (
            board[old_i][old_j][0] == "b" or board[old_i][old_j][0] == "B"
        ):  # if the computer grab the player piece (b: player piece, B: player piece if he becomes king)
            return False
        return True
    ...
    def check_moves(board, old_i, old_j, new_i, new_j):
        ...
        if (
            board[new_i][new_j] == "---"
        ):  # if the cible position of the computer piece is  empty, the computer can do the move on this position
            return True
            ...
            for j in range(8):
                if board[i][j][0] == "c" or board[i][j][0] == "C":
                    ...
                    if i + 1 > 7 or j - 1 < 0 or i - 1 < 0 or j + 1 > 7:
                        continue
                    if (
                        board[i + 1][j - 1][0] == "b" or board[i + 1][j - 1][0] == "B"
                    ) and board[i - 1][j + 1] == "---":
                        result -= 3
                    if (
                        board[i + 1][j + 1][0] == "b" or board[i + 1][j + 1] == "B"
                    ) and board[i - 1][j - 1] == "---":
                        result -= 3
    ...
    def find_player_available_moves(board, mandatory_jumping):
        ...
        for m in range(8):
            for n in range(8):
                if (
                    board[m][n][0] == "b"
                ):  # verify if the value of the array beginnt with 'b' (player)
                    if Checkers.check_player_moves(
                        board, m, n, m - 1, n - 1
                    ):  # verify if the current move is authorized (cible position is Top left from current position)
                        available_moves.append(
                            [m, n, m - 1, n - 1]
                    ...
                    if Checkers.check_player_moves(
                        ...
                        available_moves.append([m, n, m - 1, n + 1])
                    if Checkers.check_player_jumps(
                        board, m, n, m - 1, n - 1, m - 2, n - 2
                    ):  # jump the player piece over the computer piece (cible position is Top left from current position)
                        available_jumps.append([m, n, m - 2, n - 2])
                    if Checkers.check_player_jumps(
                        board, m, n, m - 1, n + 1, m - 2, n + 2
                    ):  # jump the player piece over the computer piece (cible position is Top right from current position)
                        available_jumps.append([m, n, m - 2, n + 2])
                elif (
                    board[m][n][0] == "B"
                ):  # # verify if the value of the array beginnt with 'B' (player has becommed king)
                    if Checkers.check_player_moves(
                        board, m, n, m - 1, n - 1
                    ):  # verify if the current move is authorized (cible position is Top left from current position)
                        available_moves.append(
                    ...
                    if Checkers.check_player_jumps(
                        board, m, n, m + 1, n - 1, m + 2, n - 2
                    ):  # jump the player piece over the computer piece (cible position is bottom left from current position)
                        available_jumps.append([m, n, m + 2, n - 2])
                    if Checkers.check_player_moves(
                        board, m, n, m + 1, n + 1
                    ):  # verify if the current move is authorized (cible position is bottom right from current position)
                    ...
                    if Checkers.check_player_jumps(
                        board, m, n, m + 1, n + 1, m + 2, n + 2
                    ):  # jump the player piece over the computer piece (cible position is bottom right from current position)
                        available_jumps.append([m, n, m + 2, n + 2])
        if mandatory_jumping is False:  # if mandatory jumping is not activated
            available_jumps.extend(
                available_moves
            )  # add elements of  available_moves to available_jumps. the player can do a jump or a move
            return available_jumps
        elif mandatory_jumping is True:  # if mandatory jumping is activated
            if (
                len(available_jumps) == 0
            ):  # if not jump over computer available, then use the player must play his move
                return available_moves
    ...
    def check_player_moves(board, old_i, old_j, new_i, new_j):
        ...
        if (
            board[old_i][old_j] == "---"
        ):  # if the start position of the player piece is empty
            return False
        if (
            board[new_i][new_j] != "---"
        ):  # if the cible position of the player piece is not empty
            return False
        if (
            board[old_i][old_j][0] == "c" or board[old_i][old_j][0] == "C"
        ):  # if the player grab the computer piece (c: computer piece, C: computer piece if he becomes king)
            return False
        if (
            board[new_i][new_j] == "---"
        ):  # if the cible position of the player piece is  empty, the player can do the move on this position
            return True
    ...
    def check_player_jumps(board, old_i, old_j, via_i, via_j, new_i, new_j):
        ...
        if (
            ...
            return False
        if (
            board[via_i][via_j][0] == "B" or board[via_i][via_j][0] == "b"
        ):  # if the piece, that has been jumped is for the player
            return False
        if (
            board[new_i][new_j] != "---"
        ):  # if the cible position of the player piece is not empty
    ...
    def evaluate_states(self):
        ...
        t1 = (
            time.time()
        )  # get the current time to calculate the time reaction of algorithm
        current_state = Node(
            deepcopy(self.matrix)
        )  # copy the current gameboard and use Node to get the availables moves and jumps of the computer in the current gameboard

        first_computer_moves = current_state.get_children(
            True, self.mandatory_jumping
        )  # # get Node instance of the current availables moves or jumps of maximizing player
        if (
            len(first_computer_moves) == 0
        ):  # if not availables moves or jumps of maximizing player, then count the game pieces of both player
            if (
                self.player_pieces > self.computer_pieces
            ):  # if the player has more piece than the computer, then the player won the game
                ...
                self.message = (
                    "Computer has no available moves left. --- GAME ENDED! ---"
                )
                self.observerController.notifyObserverViewCheckers(message=self.message)
                _thread.start_new_thread(self.observerController.doAudio, (TIEGAME,))
            ...
            for i in range(len(first_computer_moves)):
                child = first_computer_moves[
                    i
                ]  # get child of Node class with availables jumps or moves
                value = Checkers.minimax(
                    child.get_board(),
                    4,
                    -math.inf,
                    math.inf,
                    False,
                    self.mandatory_jumping,
                )  # use alpha beta pruning algorithm and minimax with all array gameboard of the Node kind instance with a depth of 4 through these array gameboards
                dict[value] = child
            if len(dict.keys()) == 0:  # not moves or jumps availables
                self.message = (
                    "Computer has cornered itself. :) :) :) YOU WIN! (: (: (:"
                )
                self.final_score[0] += 1
                self.observerController.notifyObserverViewCheckers(
                    newScore=self.final_score, message=self.message
                ...
                _thread.start_new_thread(
                    self.observerController.doAudio, (FRANKA_PLAY,)
                )
                self.observerController.notifyObserverViewCheckers(message=self.message)
    ...
    def minimax(board, depth, alpha, beta, maximizing_player, mandatory_jumping):
        ...
        if (
            depth == 0
        ):  # if depth is null, then get the evaluation value of end of node of tree
            return Checkers.calculate_heuristics(
                board
            )  # heuristic evaluation of all the position in the checker gameboard
        ...
        if maximizing_player is True:  # maximizer player
            ...
            for child in current_state.get_children(True, mandatory_jumping):
                ev = Checkers.minimax(
                    child.get_board(), depth - 1, alpha, beta, False, mandatory_jumping
                )
            ...
            for child in current_state.get_children(False, mandatory_jumping):
                ev = Checkers.minimax(
                    child.get_board(), depth - 1, alpha, beta, True, mandatory_jumping
                )
    ...
    def make_a_move(board, old_i, old_j, new_i, new_j, big_letter, queen_row):
        ...
        letter = board[old_i][old_j][
            0
        ]  # get the current letter (b for player, B for king player, c for computer or C for king computer)
        i_difference = old_i - new_i
        j_difference = old_j - new_j
        if (
            i_difference == -2 and j_difference == 2
        ):  # if the cible position of the jump is bottom left
            board[old_i + 1][
                old_j - 1
            ] = "---"  # the jumpy place will be empty (jumpy piece will be removed)

        elif (
            i_difference == 2 and j_difference == 2
        ):  # if the cible position of the jump is top left
            board[old_i - 1][old_j - 1] = "---"

        elif (
            i_difference == 2 and j_difference == -2
        ):  # if the cible position of the jump is top right
            board[old_i - 1][old_j + 1] = "---"

        elif (
            i_difference == -2 and j_difference == -2
        ):  # if the cible position of the jump is bottom right
            board[old_i + 1][old_j + 1] = "---"

        if (
            new_i == queen_row
        ):  # if the cible position of the player or computer is the index (0 or 7), then the player or computer become king
            letter = big_letter
        board[old_i][
            old_j
        ] = "---"  # if current player do a jump, the start position of the piece will be empty
        board[new_i][new_j] = (
            letter + str(new_i) + str(new_j)
        )  # if the current player does a jump, the cible position of the piece will receive the moved piece
    ...
    def play(self, fromInd, toInd, mandatory_jumping):
        ...
        viaPiece = (
            []
        )  # array of the list of the position of the piece, that has been jumped
        self.mandatory_jumping = mandatory_jumping
        matrixFrom = (
            self.matrix
        )  # copy the array of curent state of gameboard from controller to matrixFrom
        if self.player_turn is True:  # verify if player turn
            self.message = "Player's turn."
            self.observerController.notifyObserverViewCheckers(
                message=self.message
            )  # notify the appController, that the player's turn
            _thread.start_new_thread(
                self.observerController.doAudio, ("Player's turn!",)
            )

            verify = self.get_player_input(
                fromInd, toInd
            )  # transfer the player's moves to (fromInd: start position of game piece and toInd: cible position of game piece)
            if verify == True:  # if the move of player is authorized
                ...
                _thread.start_new_thread(
                    self.observerController.doAudio, ("Computer's turn!",)
                )
                self.observerController.notifyObserverViewCheckers(
                    message=self.message
                )  # notify the appController, that the computer's turn
                move1, move2, move3, move4 = self.evaluate_states()  # get computer move
                matrixTo = (
                    self.matrix
                )  # copy the current matrix after the computer has done a move

                indexs1 = np.flatnonzero(
                    np.char.startswith(matrixFrom, "b")
                )  # get array of all index position of player piece of Previous Array
                indexs2 = np.flatnonzero(
                    np.char.startswith(matrixTo, "b")
                )  # get  array of all index position of player piece after that the computer has done a move

                # compare both indexs to get the player piece that has been jumped
                if len(indexs1) != len(indexs2):  # if both index are not egal
                    arraydiffer = np.setdiff1d(
                        matrixFrom, matrixTo
                    )  # get the  difference of two arrays and return the unique values in matrixFrom that are not in matrixTo.
                    indexs = np.flatnonzero(
                        np.char.startswith(arraydiffer, "b")
                    )  # get array of index of a value that begint with "b" (jumped player piece)
                    for i in indexs:
                        viaPiece.append(
                            str(arraydiffer[i])[1:]
                        )  # get x,y index coordinate position of the jumped player piece and save it into array viaPiece

                self.player_turn = not self.player_turn  # player turn

                if (
                    self.player_pieces == 1
                ):  # if  player has not pieces available, then computer win the game
                    self.message = (
                        "You have no pieces left. :( :( :( YOU LOSE! ): ): ):"
                    )
                ...
                ):  # if  computer has not pieces available, then player win the game
                    self.message = (
                        "Computer has no pieces left. :) :) :) YOU WIN! (: (: (:"
                    )
                    self.final_score[0] += 1
                    time.sleep(3)
                    self.observerController.notifyObserverViewCheckers(
                    ...
                    _thread.start_new_thread(
                        self.observerController.playAudio, (GAMEWIN,)
                    )
                    _thread.start_new_thread(
                        self.observerController.doAudio, (WIN_PLAYER,)
                    )
                ...
                new_j = new[1]
                logger.debug(
                    "Checkers: Player play from b{}{} to b{}{}".format(
                        old_i, old_j, new_i, new_j
                    )
                )
                logger.debug(
                    "Checkers: computer play from b{}{} to b{}{}".format(
